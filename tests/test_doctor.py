...
def test_multi_slot_warns_but_passes(monkeypatch: pytest.MonkeyPatch) -> None:
    """The wire-level witness for every determinism warning (ADR 0020).

    The decision itself is tested in ``tests/test_llm_props.py`` against dict
    literals. What only the assembled CLI can prove is the wiring: that
    ``client.props()`` reaches ``determinism_warnings``, that the lines land on
    STDERR so stdout stays scriptable, and that a warning does not fail the
    run. Deleting the loop in ``cli.py``, or redirecting it to stdout, must go
    red here -- no test in the pure suite would notice either.
    """
