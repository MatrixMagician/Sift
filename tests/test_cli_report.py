...
ADR 0019: the unwritable-``--out`` branch is ``run_report``'s and moved to
``tests/test_commands_report.py``. What is left is the CLI's own surface — the
``--format`` enum (an unknown value is rejected by Typer and never reaches the
body), the assembled stdout path, and the exit-1 codes reaching the shell.
