...
**Core Value:** Turn a directory of raw diagnostics into a structured, evidence-cited triage report — entirely offline, with every claim citing verifiable event IDs (the anti-hallucination mechanism is load-bearing, not polish).
...
Issues live as GitHub issues on `MatrixMagician/Sift`, driven via the `gh` CLI. See `docs/agents/issue-tracker.md`.
...
Single-context: `SPEC.md` is authoritative, ADRs live in `docs/decisions/`. See `docs/agents/domain.md`.
