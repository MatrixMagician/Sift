...
Sift turns a directory of raw diagnostic artefacts (MicroStrategy `DSSErrors` logs, EU-stack
thread dumps, journald exports, generic application logs) into a ranked, evidence-cited triage
report — entirely offline. It is a Typer CLI (`sift`) over a five-stage batch pipeline, with all
...
```
                    src/sift/cli.py  (Typer: new, ingest, show, analyze, report, mcm, eval, doctor)
...
adapters/     pipeline/    pipeline/       pipeline/      render/       eval/
 ...
 eustack                                  mcm_facts.py    mcm_report.py judge.py
 journald                                                 pdf.py        thresholds.py
 genericlog
   │              │           │               │              │             │
   └──────────────┴───────────┴───────┬───────┴──────────────┴─────────────┘
                                      ▼
...
| Render | `render/` | `render_markdown`, `render_json`, `render_pdf`, and the separate MCM bundle in `render/mcm_report.py`. Pure functions of an open `CaseStore` — no client is constructed, so no re-inference can occur. |

`pipeline/mcm.py` and `pipeline/mcm_facts.py` sit alongside these: `analyse_mcm` is a fully
deterministic MicroStrategy memory-contract-manager analyser over stored events (no LLM, no
network), and `render_mcm_facts` renders its findings as citable `[evt:…]` fact lines spliced into
the triage prompt.
...
Adding adapter #5 requires exactly a new module plus one registration line.
...
It contains the representative exemplar id of each ranked cluster rendered as an
`[evt:<event_id>] <message>` line, plus the ids `render_mcm_facts` actually printed. Retrieved KB
chunks are spliced into a delimited block in the template but are **never** added to
`prompted_ids`; MCM facts are the deliberate inverse.
...
`judge.md`. Changing a prompt never requires touching Python.

`triage.md` carries two delimited, optional blocks marked with HTML comments — a KB block and an
MCM block. `hypothesise._apply_kb_block` and `_apply_mcm_block` either splice content into the slot
and strip the markers, or remove the whole block residue-free when there is nothing to inject. Each
...
├── pipeline/        Stage logic: dedup, cluster, retrieve, salience, hypothesise, mcm
├── llm/             The only HTTP: InferenceClient + PromptBudget
├── prompts/         Versioned Markdown prompt templates (package data)
├── render/          Pure store→text renderers: markdown, json, pdf, mcm bundle
